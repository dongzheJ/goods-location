...
function App() {
  ...
  return (
    <div className="w-full h-[100dvh] relative overflow-hidden bg-slate-950 text-slate-100 font-sans selection:bg-blue-500/30">
      {/* Header */}
      <header className="absolute top-0 left-0 right-0 z-[2010] flex flex-wrap items-center justify-between gap-3 px-4 py-3 md:top-4 md:left-4 md:right-4 md:rounded-2xl bg-slate-900/80 md:bg-slate-900/60 backdrop-blur-xl border-b md:border border-slate-800 shadow-2xl pointer-events-auto">
      ...
      <main className="absolute inset-0 z-0 w-full h-full pointer-events-auto">
        ...
        <aside className={`absolute bottom-0 left-0 right-0 md:top-28 md:bottom-6 md:left-6 md:right-auto md:w-80 z-[2000] max-h-[45vh] md:max-h-none bg-slate-900/95 md:bg-slate-900/80 backdrop-blur-xl border-t md:border border-slate-700/50 p-3 md:p-6 flex flex-col gap-3 md:gap-6 overflow-y-auto rounded-t-3xl md:rounded-2xl shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.7)] md:shadow-2xl transition-all duration-500 ease-out pointer-events-auto ${isSidebarActive ? 'translate-y-0 opacity-100' : 'translate-y-[120%] md:translate-y-0 opacity-0 md:opacity-100 pointer-events-none md:pointer-events-auto'}`}>
          {searchResult && (
            <div className="bg-slate-800/80 rounded-2xl p-3 md:p-5 border border-blue-500/30 shadow-xl animate-in fade-in slide-in-from-left-4 duration-500">
              ...
              <h3 className="text-base md:text-lg font-bold mb-0.5 md:mb-1">{searchResult.name}</h3>
              <p className="text-slate-400 font-mono mb-3 md:mb-4 text-xs xl:text-sm">SKU: {searchResult.id}</p>
              
              <div className="space-y-2 md:space-y-3">
                <div className="flex items-center justify-between p-2 md:p-3 bg-slate-900/50 rounded-xl border border-slate-700">
